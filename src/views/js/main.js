...
var resizePizzas = function(size) { 
  ...
  function changeSliderLabel(size) {
    ...
    switch(size) {
      case "1":
        pizza.textContent = "Small";
        return;
      case "2":
        pizza.textContent = "Medium";
        return;
      case "3":
        pizza.textContent = "Large";
        return;
      default:
  ...
  function changePizzaSizes(size) {
    ...
    // Iterates through pizza elements on the page and changes their widths (using percentages).
    var pizzaSize = (sizeSwitcher(size) * 100) + '%';
...
// Store the sliding pizzas in a variable so we can reuse it when making it slide.
var movingPizzas;
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var mvpizza = document.getElementById("movingPizzas1");
  for (var i = 0; i < 50; i++) {
    ...
    // Setting left 0 so we can transform.
    elem.style.left = 0;
    mvpizza.appendChild(elem);
  }
